  ...
  return <div style={{ "--accent": content.accentColor, "--accent-dark": content.accentColor } as React.CSSProperties}>
    ...
    <main id="main-content">
      <section id="home" className="hero" aria-labelledby="hero-title"><div className="container hero-grid"><div>
        ...
        <div className="hero-actions">{content.portfolioUrl ? <ExternalLink className="button" href={content.portfolioUrl}>View portfolio <span aria-hidden="true">↗</span></ExternalLink> : <a className="button" href="#work">View selected work <span aria-hidden="true">↓</span></a>}<a className="button button-secondary" href="#contact">Let’s connect</a></div>
      ...
      {content.portfolioUrl ? <section className="section portfolio-callout" aria-labelledby="portfolio-title"><div className="container callout-card"><div><p className="eyebrow">Selected work</p><h2 id="portfolio-title">See the full design portfolio in Figma.</h2><p>Explore detailed projects, process work, and interface design in one curated portfolio file.</p></div><ExternalLink className="button button-light" href={content.portfolioUrl}>Open Figma portfolio <span aria-hidden="true">↗</span></ExternalLink></div></section> : null}
      {content.education.length ? <section id="education" className="section section-tinted" aria-labelledby="education-title"><div className="container"><div className="section-heading"><div><p className="eyebrow">Background</p><h2 id="education-title">Education</h2></div></div><div className="education-grid">{content.education.map(item => <article key={item.id}><p className="education-year">{item.year}</p><h3>{item.qualification}</h3><p>{item.institution}</p><span>{item.detail}</span></article>)}</div></div></section> : null}
      <section id="contact" className="section contact-section" aria-labelledby="contact-title"><div className="container contact-grid"><div><p className="eyebrow">Let’s connect</p><h2 id="contact-title">Interested in Rumky’s work?</h2></div><div className="contact-actions">{content.email ? <><a className="email-link" href={`mailto:${content.email}`}>{content.email} <span aria-hidden="true">↗</span></a><p>Email is the preferred contact method. No phone number is displayed.</p></> : <p>Connect through LinkedIn to discuss design work and opportunities.</p>}<div className="hero-actions"><ExternalLink className="button" href={content.linkedinUrl}>View LinkedIn <span aria-hidden="true">↗</span></ExternalLink>{content.resumeUrl ? <a className="button button-secondary" href={content.resumeUrl} target="_blank" rel="noreferrer">Download résumé</a> : null}</div></div></div></section>
    </main>
