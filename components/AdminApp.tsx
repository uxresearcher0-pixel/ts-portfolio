...
type Section = "dashboard" | "profile" | "about" | "experience" | "skills" | "education" | "media" | "links" | "settings";
...
export function AdminApp({ initialAuthenticated, initialContent }: Props) {
  ...
  const completeness = useMemo(() => {
    if (!content) return 0;
    const checks = [content.name, content.title, content.introduction, content.aboutTitle, content.email, content.linkedinUrl, content.portfolioUrl, content.resumeUrl, content.profileImage, content.experiences.length, content.skills.length, content.education.length];
...
function Dashboard({content,completeness,go}:{content:PortfolioContent;completeness:number;go:(id:Section)=>void}) { const missing=[!content.profileImage&&"Profile image",!content.portfolioUrl&&"Portfolio link",!content.resumeUrl&&"Résumé link",content.email==="email@example.com"&&"Real email",content.experiences.some(x=>x.company.startsWith("Add"))&&"Verified experience"].filter(Boolean); return <><div className="dashboard-heading"><div><p className="eyebrow">Overview</p><h1>Welcome back, {content.shortName.split(" ")[0]}.</h1><p>Manage the content and presentation of your portfolio.</p></div><div className="completion-ring" style={{"--progress":`${completeness * 3.6}deg`} as React.CSSProperties}><strong>{completeness}%</strong><span>complete</span></div></div><div className="metric-grid"><Metric label="Experience" value={content.experiences.length} note="published entries"/><Metric label="Skill groups" value={content.skills.length} note="capability areas"/><Metric label="Education" value={content.education.length} note="qualifications"/><Metric label="Last update" value={content.updatedAt?new Date(content.updatedAt).toLocaleDateString():"—"} note="published content"/></div><div className="dashboard-grid"><section className="dashboard-card"><div className="card-heading"><div><h2>Content checklist</h2><p>Complete these items before sharing the portfolio.</p></div><span className={missing.length?"pill warning":"pill"}>{missing.length?`${missing.length} remaining`:"Complete"}</span></div>{missing.length?<ul className="checklist">{missing.map(item=><li key={String(item)}><span aria-hidden="true">○</span>{item}</li>)}</ul>:<div className="empty-success">✓ Core portfolio content is ready.</div>}</section><section className="dashboard-card"><h2>Quick actions</h2><div className="quick-actions"><button onClick={()=>go("media")}>▧ Add profile image <span>→</span></button><button onClick={()=>go("experience")}>▣ Update experience <span>→</span></button><button onClick={()=>go("links")}>↗ Add portfolio links <span>→</span></button><Link href="/" target="_blank">◉ Preview live site <span>↗</span></Link></div></section></div></>; }
